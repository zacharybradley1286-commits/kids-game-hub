...
// Simple wandering AI — no aggro, just roams
class PassiveAI {
  constructor(mob, worldData) {
    this.mob = mob
...
export class PassiveSpawner {
  constructor(scene, worldData, inventory, itemRegistry) {
    this.scene = scene
    this.worldData = worldData
  ...
  update(dt) {
    ...
    if (this.mobs.length >= MAX_PASSIVE) return
    this._spawnTimer += dt
    if (this._spawnTimer < 8) return  // one every 8s at day
    this._spawnTimer = 0
    this._trySpawn()
  }
  ...
  _trySpawn() {
    ...
    for (let a = 0; a < attempts; a++) {
      ...
      const ai = new PassiveAI(mob, this.worldData)
